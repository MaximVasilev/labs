...
BigInt BigInt::Read(char* fileName,int b)
	{
		value = ReadFile(fileName,b);
		base = b;
		return *this;
	}
BigInt BigInt::Write(char* fileName)
	{
...
//=======================
int* BigInt::ReadFile(char* fileName,int base)
{
	int *mas = NULL;
    char* flag;
    if(base == 10) flag = "r";
...
void BigInt::WriteFile(int *mas,char *fileName,int base)
{
    char* flag;
...
int* BigInt::Pow(int* a,int* b,int* c,int base)
 {
	bool A_negative = CheckNegative(a);
	bool B_negative = CheckNegative(b);
	bool paritet = false;
