...
class DES {
    ...
    public void init (String mode,String keyFile,String msgFile, String cphFile) throws Exception{
        theKey = readBytes(keyFile);
        subKeys = getSubkeys(theKey);
        theMsg = readBytes(msgFile);
        theCph = cipher(theMsg,subKeys,mode);
        if (!mode.equals("encrypt")){
        	 int i;
        	 i=(byte)theCph[theCph.length-1];
        	 if ((i<8)&&(i!=0))
        		 theCph=Arrays.copyOfRange(theCph, 0, theCph.length-i);
        }
    }
    ...
    public byte[] cipher(byte[] theMsg, byte[][] subKeys, 
    		  ...
    		  int i,j,blocks;
    		  blocks=theMsg.length/8;
    		  if (blocks*8<theMsg.length)
    			 blocks++; 
    		  byte[] l,r,rBackup,lr=new byte[blocks*8],temp;
    	      if (theMsg.length<8) 
    	         throw new Exception("Message is less than 64 bits.");
    	      
    	      byte[] blockArray=new byte[blocks*8];
    	      System.arraycopy(theMsg, 0, blockArray,0 ,theMsg.length);
    	      for (i=theMsg.length;i<blockArray.length;i++)
    	    	  blockArray[i]=(byte)(blockArray.length-theMsg.length);
    	      for (i=0;i<blocks;i++){
    	    	  for (j=0;j<8;j++)
    	    		  theMsg[j]=blockArray[j+8*i];
	    	      temp = selectBits(theMsg,IP);
	    	      int blockSize = IP.length;
	    	      l = selectBits(temp,0,blockSize/2);
	    	      r = selectBits(temp,blockSize/2,blockSize/2);
	    	      int numOfSubKeys = subKeys.length;
	    	      for (int k=0; k<numOfSubKeys; k++) {
	    	      	 rBackup = r;
	    	         r = selectBits(r,E);
	    	         if (mode.equalsIgnoreCase("encrypt")) 
	    	            r = doXORBytes(r,subKeys[k]);
	    	         else 
	    	      ...
	    	      byte[] lplusr = concatenateBits(r,blockSize/2,l,blockSize/2);
	    	      lplusr = selectBits(lplusr,INVP);
	    	      System.arraycopy(lplusr, 0, lr, 8*i, 8);
    	      }
    	      return lr;
    	   }
    ...
    private byte[] rotateLeft(byte[] in, int len, int step) {
     ...
     private byte[] concatenateBits(byte[] a, int aLen, byte[] b, 
        int bLen) {
     ...
     private byte[] selectBits(byte[] in, int pos, int len) {
      ...
      private int getBit(byte[] data, int pos) {
       ...
       private byte[] readBytes(String in) throws Exception {
       ...
       public void writeBytes(byte[] data, String out)
    		      throws Exception {
...
public class CipherDES {
	public static void main(String[] args) {
	      if (args.length<4) {
	          System.out.println("Incorrect parameters. Read README");
	          return;
	       }
	       DES des = new DES();
	       try {
	    	   des.init(args[0], args[1], args[2], args[3]);
	           des.writeBytes(des.theCph,args[3]);
	       } catch (Exception e) {
	          e.printStackTrace();
	          return;
	       }
